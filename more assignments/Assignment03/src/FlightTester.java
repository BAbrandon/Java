	...
	public static void main(String[] args) throws Exception {
		...
		while (fromFile.hasNext()) // need to change condition
		{
			...
			flightNumber = Integer.parseInt(flightNumberText);
				...
				// from the data file
allFights[i] = new Flight(airline, origin, destination, flightNumber);
		...
		allFights[number].setAirline(airline);
		allFights[number].setOrigin(origin);
		allFights[number].setDestination(destination);
		allFights[number].setFlightNumber(flightNumber);
		...
		output += "(";
		output += allFights[number].getAirline();
		output += allFights[number].getOrigin();
		output += allFights[number].getDestination();
		output += allFights[number].getFlightNumber();
		output += ")";
		JOptionPane.showMessageDialog(null, "New flight info for number"+
		number + "\n\n"+ output);
