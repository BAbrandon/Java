...
public class PizzaControls extends JPanel {
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			if(!gotName){
				...
				if(!text.equals("")){
					...
					JOptionPane.showMessageDialog(null, "Error! You must enter customer name", "Customer Name Error", JOptionPane.ERROR_MESSAGE);
				}
				if(gotName){
					selection = cboPizzaSize.getSelectedIndex();
					if(selection == 0){//large
						output1 = output1 + pizzaSize[0] + " ";
						gotLarge = true;
					}else if (selection == 1){//medium
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			if(!gotSize){
				JOptionPane.showMessageDialog(null, 
						"Error! You must select a size of pizza",
				         "Pizza size Error", JOptionPane.ERROR_MESSAGE);
			}else{
				
				selection = cboPizzaStyle.getSelectedIndex();
				
				switch(selection){
				case 0  : output1 = output1 + pizzaStyle[0] + " pizza with\n "; gotHandTossed = true; break;
				case 1  : output1 = output1 + pizzaStyle[1] + " pizza with\n "; gotThin = true; break;
				case 2  : output1 = output1 + pizzaStyle[2] + " pizza with\n "; gotDeepDish = true; break;
				default : output1 = output1 + "Uknown Style!";
				}//end of switch
			}
			gotStyle = true;
	...
	private class Toppings2Listener implements ItemListener
	{
		...
		{	
			...
			if(cbxMush.isSelected()){gotMush = true;}
			if(cbxPine.isSelected()){gotPine = true;}
			if(cbxOnion.isSelected()){gotOnion = true;}
			if(cbxGrPep.isSelected()){gotGrPep = true;}
		}   // end method itemStateChanged
